class Ability
  ...
  def initialize(user)
    ...
    if user.has_role?(:venue_manager, :any)
      ...
      can [:update, :destroy], Package do |package|
        user.has_role?(:venue_manager, package.venue)
      end

      can :show, Voucher do |voucher|
        user.has_role?(:venue_manager, voucher.package.venue)
      end
    ...
    if user.has_role?(:admin)
      ...
      can :read, :all
      can [:subscribe_user, :subscribe, :unsubscribe_user, :unsubscribe, :favorite_teams, :get_teams_by_sport], Team
