class Voucher < ActiveRecord::Base
  ...
  def self.redeemable(current_user)
    ...
    if creatorParty.present?
      ...
      reserved_vouchers = where(party_id: reservedPartyIDs).group_by(&:party_id)
      
      reserved_vouchers.try(:each) do |rv|

          user_voucher = where("user_id = ? AND redeemed_at IS NOT = ? AND party_id = ? ",  current_user.id, nil, rv.first.party_id)
