class User < ActiveRecord::Base
  ...
  def get_accepted_parties
    ...
    if self.admin?
      ...
      pending_parties =  Party.where("(who_created_location = 'customer_venue' OR who_created_location = 'venue_venue') AND verified = true AND is_cancelled = false").where('scheduled_for >= ?', DateTime.now.new_offset('-05:00'))
